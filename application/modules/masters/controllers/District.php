...
class District extends MX_Controller 
{
	function __construct() 
	{
	    ...
	    $this->load->library(array('Session_check','session','form_validation','validationdigi','errormsgs','encryption','authorization'));
	     $this->load->model(array('District_model','settings/Comman_model'));
	...
	public function districtStatus()
	{
		...
		{	
			...
			$params = array();
			$id = isset($_POST['district_id'])?$this->db->escape_str(trim($_POST['district_id'])):'';
			$params['status'] = isset($_POST['status'])?$this->db->escape_str(trim($_POST['status'])):'';
			$params['district_id'] = $this->encryption->decrypt("$id");
			$result =$this->District_model->districtStatusUpdate($params);
	...
	public function editDistrict()
	{
		...
		{	
			...
			$district_id =isset($_POST['district_id'])?$this->db->escape_str(trim($_POST['district_id'])):''; 
			$params['district_id'] = $this->encryption->decrypt("$district_id");
			$data =$this->District_model->getDistrictList($params);
			...
			$this->load->view('edit_district',$view_data);
		}
	}
	public function district()
	{
		...
		{ 
			...
			if($this->form_validation->run() == TRUE)
			{
				 ...
				 #create new array
				$params['district_name'] = isset($_POST['district_name'])?$this->db->escape_str(trim($_POST['district_name'])):'';
				$params['state_id'] = isset($_POST['state_id'])? (int)$_POST['state_id']:'';
				$params['country_id'] = isset($_POST['country_id'])? (int)$_POST['country_id']:'';
				$params['district_code'] = isset($_POST['district_code'])?$this->db->escape_str(trim($_POST['district_code'])):'';
				#UPDATE from edit country
				$id = isset($_POST['district_id'])?$this->db->escape_str(trim($_POST['district_id'])):'';
				$params['district_id'] = $this->encryption->decrypt("$id");
				$result  =$this->District_model->saveDistrict($params);
	...
	public function ajax_list()
	{
		...
		foreach ($list as $district) 
		{
        	...
        	$district_id = $this->encryption->encrypt($district->district_id);
        	$editonclick = "districtEdit('".$district_id."');";
        	$statusclick = "districtStatus('".$i."');";
        	
			$no++;
			$row = array();
			$row[] = $no;
			$row[] = $district->country_name;
			$row[] = $district->state_name;
			$row[] = $district->district_name;
			$row[] = $district->district_code;
			$row[] = $district->status;
			$row[] ='
					<div class="btn-group">
					...
					<form id="status_frm_id_'.$i++.'" method="post" action="'.base_url().'masters/district/districtStatus">
